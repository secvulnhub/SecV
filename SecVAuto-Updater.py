...
class SecVUpdater:
    ...
    def check_for_updates(self) -> Tuple[bool, str]:
        ...
        try:
            ...
            # Get the latest tag info from GitHub
            url = f"{GITHUB_API}/tags"  # Changed endpoint from "/releases/latest" to "/tags"
            with urllib.request.urlopen(url, timeout=10) as response:
                data = json.loads(response.read())
                # The /tags endpoint returns a list, so we take the first item (which is the newest)
                if data:
                    # The key for the version name is 'name' in the tags response
                    self.latest_version = data[0]['name'].lstrip('v')
                else:
                    print(f"{RED}✗ No version tags found in the repository.{NC}")

...
# ============================================================================
# Install optional network tools (masscan)
# ============================================================================
...
if [ "$HAS_GO" = true ]; then
    ...
    if [ -f go.mod ]; then
        echo -e "${CYAN}[*] Resolving Go dependencies...${NC}"
        go mod tidy 2>&1 || true
    fi

    echo -e "${CYAN}[*] Compiling...${NC}"
    if go build -ldflags="-s -w" -o secV .; then
        chmod +x secV
    ...
    echo -e "${DIM}    Go is required to run SecV${NC}"
fi
echo
...
if [ "$SYSTEM_WIDE" = true ]; then
    echo -e "  ${GREEN}secV${NC}              # start"
else
    echo -e "  ${GREEN}./secV${NC}            # start"
fi
echo -e "  ${DIM}help              # commands${NC}"
echo -e "  ${DIM}show modules      # list modules${NC}"
echo -e "  ${DIM}update            # pull latest${NC}"
echo

echo -e "${DIM}Use only on systems you have permission to test.${NC}"
echo
